...
async function fetchData(location) {
  try {
    ...
    const res = await fetch(
      `${DAILY_BASE_URL + location}&key=${DAILY_API_KEY}`,
    );
    ...
    const dailyForecastData = dalyData.data;
    ...
    dailyForecastData.forEach((dayData) => {
      ...
      const { description } = dayData.weather;
      ...
      const htmlContent = `
            ...
            <div class="temp">${maxTemp.toFixed(0)} / ${minTemp.toFixed(
  0,
)}</div>
          `;
    ...
    document.getElementById('hero-description').textContent = data.list[0].weather[0].description;
    ...
    document.getElementById('real-feel').innerHTML = `${felsLikeCelsius.toFixed(
      0,
    )}&deg;`;

    document.getElementById(
      'humidity',
    ).innerHTML = `${data.list[0].main.humidity}%`;
    const windSpeedMetersPerSecond = data.list[0].wind.speed;
    const windSpeedKmPerHour = windSpeedMetersPerSecond * 3.6;
    document.getElementById(
      'wind-speed',
    ).innerHTML = `${windSpeedKmPerHour.toFixed(0)}km/h`;
